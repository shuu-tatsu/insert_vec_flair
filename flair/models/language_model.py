...
class LanguageModel(nn.Module):
    ...
    def get_context_vec(self, is_forward_lm):
        ...
        tensor_h0_list = []
        tensor_c0_list = []
        for np in np_list[0]: #hx
            tensor_h0_list.append(torch.from_numpy(np))
        for np in np_list[1]: #cx
            tensor_c0_list.append(torch.from_numpy(np))

        return tensor_h0_list, tensor_c0_list

    def get_avg_vec(self, context_h0_list, context_c0_list, no_list, denominator):
        index_list = random.sample(no_list, denominator)
        temp_h0 = torch.zeros(context_h0_list[0].size())
        temp_c0 = torch.zeros(context_c0_list[0].size())
        for index in index_list:
            temp_h0 = torch.add(temp_h0, context_h0_list[index])
            temp_c0 = torch.add(temp_c0, context_c0_list[index])
        avg_h0 = temp_h0 / denominator
        avg_c0 = temp_c0 / denominator
        return avg_h0, avg_c0
    # fix end
    '''
    def get_representation(self, strings: List[str], chars_per_chunk: int = 512):
    '''
    def get_representation(self, strings: List[str], ne_sent_index_list, is_forward_lm, chars_per_chunk: int = 512):
        ...
        # hidden: (h_0, c_0)
        # hidden[0]: torch.Size([1, 32, 2048])
        # hidden[0][0][i]に平均文脈ベクトルを挿入
        # hidden[0][0][i].size(): torch.Size([2048])
        hidden = self.init_hidden(len(chunks[0]))
        # fix start
        #import pdb; pdb.set_trace()
        context_h0_list, context_c0_list = self.get_context_vec(is_forward_lm)
        no_list = [i for i in range(len(context_h0_list))]
        denominator = 10
        for i in range(len(chunks[0])):
            if i in ne_sent_index_list:
                if is_forward_lm:
                    #numpy_forward_hxcx_stackedからロード
                    avg_h0, avg_c0 = self.get_avg_vec(context_h0_list, context_c0_list, no_list, denominator)
                    hidden[0][0][i] = avg_h0
                    hidden[1][0][i] = avg_c0
                else:
                    #numpy_backward_hxcx_stackedからロード
                    avg_h0, avg_c0 = self.get_avg_vec(context_h0_list, context_c0_list, no_list, denominator)
                    hidden[0][0][i] = avg_h0
                    hidden[1][0][i] = avg_c0
        # fix end
